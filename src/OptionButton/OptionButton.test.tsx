...
test('A default Option Button', () => {
  const tree = renderer.create(<OptionButton icon={testIcon} />).toJSON()
  expect(tree).toMatchSnapshot()
})

test('A on Option Button', () => {
  const tree = renderer.create(<OptionButton toggled={true} icon={testIcon} />).toJSON()
  expect(tree).toMatchSnapshot()
})

test('A dsiabled Option Button', () => {
  const tree = renderer.create(<OptionButton disabled={true} icon={testIcon} />).toJSON()
  expect(tree).toMatchSnapshot()
})
